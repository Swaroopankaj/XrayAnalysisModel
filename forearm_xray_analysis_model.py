...
def is_valid_dataset_path(path):
    ...
    # It must contain 'train', 'valid', and 'test' subdirectories,
    # and each must have 'normal' and 'abnormal' folders with at least one image.
    required_structure_exists = (
        os.path.exists(os.path.join(path, 'train', CLASSES[0])) and
        os.path.exists(os.path.join(path, 'train', CLASSES[1])) and
        os.path.exists(os.path.join(path, 'valid', CLASSES[0])) and
        os.path.exists(os.path.join(path, 'valid', CLASSES[1])) and
        os.path.exists(os.path.join(path, 'test', CLASSES[0])) and
        os.path.exists(os.path.join(path, 'test', CLASSES[1]))
    )
    
    # Also check if there's at least one image in a representative test class folder
    has_images = (os.path.exists(os.path.join(path, 'test', CLASSES[0])) and
                  len(os.listdir(os.path.join(path, 'test', CLASSES[0]))) > 0)
    
    return required_structure_exists and has_images
    ...
    if is_valid_dataset_path(current_dir):
        ...
        while DATASET_BASE_DIR is None:
            ...
            if is_valid_dataset_path(user_input_path):
                ...
                print("Invalid path or dataset structure not found. Please ensure the path is correct and contains 'train', 'valid', 'test' with 'normal' and 'abnormal' subfolders. Try again.")
